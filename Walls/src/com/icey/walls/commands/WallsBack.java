...
public class WallsBack implements CommandExecutor, TabCompleter {
	...
	public WallsBack(MainClass mainClass, ArenaManager arenaManager, WallsTool wallsTool) {
		myplugin = mainClass;
						...
						if (arenaConfig != null) {
							if (args.length >= 3) {
								if (args[2].equalsIgnoreCase("info")) {
									sender.sendMessage(arenaManager.getArenaConfig(name).listSpawns());
									sender.sendMessage(arenaManager.getArenaConfig(name).listRegions());
								}
								else if (args[2].equalsIgnoreCase("stop")){
									arenaManager.getArenaConfig(name).getArena().stopGame();
								}
								else if (args[2].equalsIgnoreCase("enable")) {
									sender.sendMessage(args[1]);
									if (arenaManager.checkConfig(args[1])) {
										arenaManager.getArenaConfig(args[1]).setEnabled(true);
